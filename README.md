...
-  **parallelStream()은 N×Q 조건에 따라 성능이 달라지며 가벼운 연산/작은 데이터에서는 오버헤드로 손해가 날 수 있다**
<br>
...
## 💡 기술적 목표
- 같은 입력 데이터에서
- (1) `parallelStream()`이 항상 성능 우위를 점하는지 확인하기 위해 **연산 비용(Q)**이 다른 두 가지 케이스를 대조군으로 설정함 (단순 연산 vs 복잡 연산)
...
- (2) **중간 연산을 여러 번 나누는 방식(Original)** vs **조건을 합쳐 중간 연산 개수를 줄이는 방식(Optimized)**
